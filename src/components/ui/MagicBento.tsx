...
const DEFAULT_GLOW_COLOR = '215, 226, 234';
...
export const MagicBentoProvider: React.FC<{
  children: React.ReactNode;
  disableAnimations?: boolean;
  enableSpotlight?: boolean;
  spotlightRadius?: number;
...
export const MagicCard: React.FC<{
  ...
  const baseClassName = `magic-bento-card ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''} ${url ? 'cursor-pointer' : ''} ${className}`;
  const cardProps = {
      ...
      <BentoCardGrid>
        {cards.map((card, index) => (
          <MagicCard
            ...
            particleCount={particleCount}
            glowColor={glowColor}
            className={textAutoHide ? 'magic-bento-card--text-autohide' : ''}
          >
