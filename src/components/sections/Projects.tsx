...
const Card = ({ project, index, totalCards }: { project: ProjectItem; index: number; totalCards: number }) => {
  ...
  return (
    <div 
      ref={containerRef}
      className="sticky top-16 sm:top-20 md:top-28 min-h-[580px] h-[85vh] md:h-[88vh] w-full flex items-center justify-center pt-3 sm:pt-6"
      style={{
        top: `calc(7vh + ${index * 18}px)`
      }}
    >
      <TiltCard 
        maxDegree={5} 
      ...
      >
        <motion.div 
          ...
          className="w-full h-full max-w-7xl mx-auto rounded-[24px] sm:rounded-[44px] md:rounded-[54px] border border-sm:border-2 border-[#D7E2EA]/80 bg-[#0C0C0C] p-4 sm:p-7 md:p-9 flex flex-col justify-between gap-3 sm:gap-6 shadow-2xl overflow-hidden"
        >
          {/* Top row */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 sm:gap-4">
            <div className="flex items-center gap-3 sm:gap-6">
              <span className="text-[#D7E2EA] font-black text-[clamp(1.8rem,5vw,4.5rem)] leading-none select-none">
                {String(index + 1).padStart(2, '0')}
              </span>
              <div className="flex flex-col">
                <span className="text-[#D7E2EA]/60 uppercase tracking-widest text-[10px] sm:text-xs md:text-sm font-semibold">{project.category}</span>
                <h3 className="text-[#D7E2EA] font-bold text-lg sm:text-2xl md:text-3xl uppercase tracking-tight">{project.title}</h3>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-between sm:justify-start">
              {project.github && (
                <a
                  ...
                  className="inline-flex items-center justify-center rounded-full border border-sm:border-2 border-[#D7E2EA]/40 text-[#D7E2EA]/80 hover:text-[#D7E2EA] hover:border-[#D7E2EA] font-medium uppercase tracking-widest px-3 py-1.5 sm:px-6 sm:py-3 text-[11px] sm:text-sm transition-all hover:scale-105 active:scale-95 gap-1.5 sm:gap-2"
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2.5 sm:gap-3 text-xs sm:text-sm">
            <p className="text-[#D7E2EA]/80 max-w-2xl text-[11px] sm:text-sm leading-relaxed line-clamp-2 md:line-clamp-none">
              {project.description}
            </p>
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              {project.tags.map((tag, tIdx) => (
                <span 
                  key={tIdx} 
                  className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full border border-[#D7E2EA]/30 bg-[#D7E2EA]/5 text-[#D7E2EA]/90 text-[9px] sm:text-xs font-mono uppercase tracking-wider"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="flex-1 flex flex-col md:flex-row gap-2.5 sm:gap-5 w-full h-full min-h-0 overflow-hidden">
            {/* Left col */}
            <div className="w-full md:w-[40%] flex flex-row md:flex-col gap-2.5 sm:gap-5 h-28 sm:h-36 md:h-full min-h-0 shrink-0 md:shrink">
              <div className="w-1/2 md:w-full h-full md:h-[45%] rounded-xl sm:rounded-[32px] overflow-hidden group">
                <img 
              ...
              <div className="w-1/2 md:w-full h-full md:h-[55%] rounded-xl sm:rounded-[32px] overflow-hidden group">
                <img 
            ...
            {/* Right col */}
            <div className="w-full md:w-[60%] rounded-xl sm:rounded-[32px] overflow-hidden h-32 sm:h-48 md:h-full min-h-0 group shrink-0 md:shrink">
              <img 
